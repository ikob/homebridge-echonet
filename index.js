...
//        limiter.removeTokens(1, function() {
        ...
        if (value == 1)
            limiter.removeTokens(1, function () {
                el.setPropertyValue(address, eoj, 0xB0, {'mode': 3});
            });
        else if (value == 2)
            limiter.removeTokens(1, function () {
                el.setPropertyValue(address, eoj, 0xB0, {'mode': 2});
            });
        else if (value == 3)
            limiter.removeTokens(1, function () {
                el.setPropertyValue(address, eoj, 0xB0, {'mode': 1});
            });
        callback();
    });
...
//                EchonetDevs.motion(className, _this.el, accessory, address, eoj, _this.log);
    ...
    EchonetPlatform.prototype.updateAccessoriesReachability = function () {
        ...
        this.accessories.forEach(function (accessory) {
            return accessory.updateReachability(false);
        });
    };
